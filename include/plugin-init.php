	...
	*/
	...
	if( !function_exists('tourmaster_table_init') ){
		function tourmaster_table_init(){
			...
			// room settings table
			$sql = "CREATE TABLE {$wpdb->prefix}tourmaster_room_settings (
				id bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT,
				room_id bigint(20) UNSIGNED DEFAULT NULL,
				date_list longtext DEFAULT NULL,
				min_date tinyint DEFAULT NULL,
				max_date tinyint DEFAULT NULL,
				buffer_number tinyint DEFAULT NULL,
				buffer_day varchar(100) DEFAULT NULL,
				PRIMARY KEY  (id)
			) {$charset_collate};";
			dbDelta($sql);
			...
			$sql = "CREATE TABLE {$wpdb->prefix}tourmaster_room_booking (
				...
				buffer boolean NOT NULL default 0,
				PRIMARY KEY  (id)
			) {$charset_collate};";
			dbDelta($sql);
			...
			$sql = "CREATE TABLE {$wpdb->prefix}tourmaster_room_review (
				...
				review_score tinyint DEFAULT NULL,
				review_detail longtext DEFAULT NULL,
				review_description longtext DEFAULT NULL,
				review_date datetime DEFAULT '0000-00-00 00:00:00' NOT NULL,
				reviewer_name varchar(100) DEFAULT NULL,
				reviewer_email varchar(100) DEFAULT NULL,
	...
	if( !function_exists('tourmaster_set_plugin_role') ){
		function tourmaster_set_plugin_role(){
			...
			foreach( $post_type_cap as $cap ){
				...
				$admin->add_cap(str_replace('%s', 'car', $cap));
			...
			$admin->add_cap('manage_car_category');
			$admin->add_cap('manage_car_tag');
			$admin->add_cap('manage_car_filter');
			$admin->add_cap('manage_car_order');
