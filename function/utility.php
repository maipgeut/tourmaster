	...
	*/
	...
	if( !function_exists('tourmaster_enqueue_icon') ){
		function tourmaster_enqueue_icon(){
			...
			$gdlr_travel = tourmaster_get_option('plugin', 'gdlr-travel', 'enable');
			if( $gdlr_travel = 'enable' ){
				wp_enqueue_style('gdlr-travel', TOURMASTER_URL . '/plugins/gdlr-travel/style.css');
			}
	...
	if( !function_exists('tourmaster_get_post_list') ){	
		function tourmaster_get_post_list( $post_type, $with_none = false ){
			$post_list = get_posts(array('post_type' => $post_type, 'numberposts'=>999,'suppress_filters'=>false));
	...
	if( !function_exists('tourmaster_get_term_list') ){	
		function tourmaster_get_term_list( $taxonomy, $cat = '', $with_all = false, $hirachy = false ){
			$term_atts = array(
				'taxonomy'=>$taxonomy, 
				'hide_empty'=>0,
				'number'=>999
			...
			if( !empty($term_list) ){
				foreach( $term_list as $term ){
					if( !empty($term->slug) && !empty($term->name) ){
						...
						if( $hirachy ){
							$ret = $ret + tourmaster_get_term_children($taxonomy, $term->term_id, '');
						}
					}
				}
			}
	...
	if( !function_exists('tourmaster_get_term_children') ){	
		function tourmaster_get_term_children( $taxonomy, $parent, $indicator = '' ){
			$indicator .= '-';
			$term_atts = array(
				'taxonomy'=>$taxonomy, 
				'hide_empty'=>0,
			...
			if( !empty($term_list) ){
				foreach( $term_list as $term ){
					if( !empty($term->slug) && !empty($term->name) ){
						$ret[$term->slug] = $indicator . ' ' . $term->name;
						$ret = $ret + tourmaster_get_term_children($taxonomy, $term->term_id, $indicator);
					}
				}
			}
