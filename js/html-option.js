	...
	******************************************/
	...
	$(document).ready(function(){
		$('.tourmaster-page-option-content').each(function(){
			...
			$('#wpbody-content').on('click', '.edit-post-header__settings, .editor-post-publish-button, .editor-post-preview, .editor-post-save-draft', function(){
				data_input.val(JSON.stringify(html_option.get_val()));
			});
...
(function($){
	...
	tourmasterHtmlOption.prototype = {
		...
		bind_input_format: function(){
			...
			this.container.on('click', 'input[data-input-type="icon"]', function(){
		...
		display_icon_list: function(element){
			...
			if( typeof(html_option_val) === 'undefined' || !html_option_val.icon_list ) return;
